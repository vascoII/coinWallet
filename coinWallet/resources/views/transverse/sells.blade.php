                        ...
                        <table class="table">
                            ...
                            <tbody>
                                <tr>
                                    ...
                                    @if($coinsTotal['value'] > ($coinsTotal['spend'] + $coinsTotal['transfer']))
                                        <td>{{ $coinsTotal['value'] - $coinsTotal['spend']  - $coinsTotal['transfer'] . ' €'}}</td>
                                        <td></td>
                                    @else
                                        <td></td>
                                        <td>{{ $coinsTotal['spend'] + $coinsTotal['transfer'] - $coinsTotal['value'] . ' €'}}</td>
                                    @endif
                                </tr>
                            </tbody>
                        </table>
                        ...
                        <table class="table">
                            ...
                            @foreach($coinsSymbols as $symbol => $value)
                                <tr>
                                    ...
                                    <td>{{ $value['value'] . ' €'}}</td>
                                    @if($value['value'] > ($value['spend'] + $value['transfer']))
                                        <td>{{ $value['value'] - $value['spend'] - $value['transfer'] . ' €'}}</td>
                                        <td></td>
                                    @else
                                        <td></td>
                                        <td>{{ $value['spend'] + $value['transfer'] - $value['value'] . ' €'}}</td>
                                    @endif
                                </tr>
                            @endforeach
                            </tbody>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            const chart = Highcharts.chart('container_total', {
                ...
                series: [{
                    name: 'Spend + Transfer',
                    data: [{{ $coinsTotal['spend'] + $coinsTotal['transfer'] }}]
                }, {
                    name: 'Gain',
        ...
        document.addEventListener('DOMContentLoaded', function () {
            const chart = Highcharts.chart('container_total_coins', {
                ...
                title: {
                    text: 'Total spend/transfer, gain and losses by Coins'
                },
                xAxis: {
                ...
                series: [{
                    ...
                    data: [@foreach($coinsSymbols as $symbol => $value)
                        {{ $value['spend'] + $value['transfer'] }},
                    @endforeach]
                }, {
                    name: 'Gain',
                    data: [@foreach($coinsSymbols as $symbol => $value)
                        @if($value['value'] > ($value['spend'] + $value['transfer']))
                        {{ $value['value'] - $value['spend'] - $value['transfer']}},
                        @else
                        0,
                        @endif
                    @endforeach]
